...
def train(sgd, model, data, epoch_size=32):
...
input1 = layers.Input(shape=(img_rows, img_cols, img_channels))
input2 = layers.Input(shape=(n_class=10,))
...
x = Activation('relu')(x)
x1 = AveragePooling2D(pool_size=(8, 8), strides=None, border_mode='valid', dim_ordering='tf')(x)
x = Flatten()(x1)

softmax = Dense(nb_classes, activation='softmax', name='softmax')(x)

conv_out=Conv2D(32 * widening_factor, (3, 3),activation='relu', padding="same")(x1)
conv_out=UpSampling2D(size=(2, 2))(conv_out)
conv_out=Conv2D(32 * widening_factor, (3, 3),activation='relu', padding="same")(conv_out)
conv_out=UpSampling2D(size=(2, 2))(conv_out)

conv_out=Conv2D(3, (3, 3), padding="same",activation='sigmoid', name="conv_out")(conv_out)
...
if not data_augmentation:
    ...
    for j in range(20):
        ...
        plot_generated_batch(j,model=model, data1=(x_train, y_train),data2=(y_test, y_test))
        # 学習履歴を保存
